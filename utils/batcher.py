...
class Example(object):
    ...
    def __init__(self, article, abstract_sentences, vocab):
        ...
        if len(article_words) > config.max_encode_len:
            article_words = article_words[:config.max_encode_len]
        ...
        self.dec_input, self.target = self.get_dec_inp_targ_seqs(abs_ids, config.max_decode_len, start_decoding,
                                                                 stop_decoding)
        ...
        _, self.target = self.get_dec_inp_targ_seqs(abs_ids_extend_vocab, config.max_decode_len, start_decoding,
                                                    stop_decoding)
...
class Batch(object):
    ...
    def init_decoder_seq(self, example_list):
        ...
        for ex in example_list:
            ex.pad_decoder_inp_targ(config.max_decode_len, self.pad_id)

        # Initialize the numpy arrays.
        self.dec_batch = np.zeros((self.batch_size, config.max_decode_len), dtype=np.int32)
        self.target_batch = np.zeros((self.batch_size, config.max_decode_len), dtype=np.int32)
        self.dec_padding_mask = np.zeros((self.batch_size, config.max_decode_len), dtype=np.float32)
...
class Batcher(object):
    ...
    def text_generator(self, example_generator):
        while True:
            ...
            try:
                article_text = str(e.features.feature['article'].bytes_list.value[0],
                                   'utf-8')  # the article text was saved under the key 'article' in the data files
                abstract_text = str(e.features.feature['abstract'].bytes_list.value[0],
